...
func updateDivisor():
	...
	GVars.wheelphase = 4	# TESTING PURPOSES ONLY, DELETE LATER.
	...
	for count in range(1, 12):
		if (GVars.wheelphase == count):
			...
			print(wheelphase_json.wheelphases.keys(), " | ", count, " | ", wheelphase_index)	# TESTING PURPOSES ONLY, DELETE LATER.
			...
			if (wheelphase_index == -1):
				# L.B: Delete this paragraph comment later once you understand.
				#	The "equivalent" of this in your old code is the final "else" statement.
				#	I chose to make this more difficult for myself by having a key name not be a number.
				#	No pattern means manually writing a case for it, which is fine.
